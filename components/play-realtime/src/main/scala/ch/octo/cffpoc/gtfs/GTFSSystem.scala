 ...
 */
class GTFSSystem(trips: Map[TripId, Trip], exceptionDates: Map[LocalDate, Set[ServiceId]]) {
...
object GTFSSystem {
  ...
  def loadTrips(rootSrc: String,
    ...
    routes: Map[RouteId, RawRoute]): Map[TripId, Trip] = {

    RawTripReader.load(s"$rootSrc/$FILENAME_TRIPS")
