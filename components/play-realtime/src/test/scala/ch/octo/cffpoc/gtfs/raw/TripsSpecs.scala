 ...
 */
class TripsSpecs extends FlatSpec with Matchers {
  behavior of "Trips"
  def load = Trips.load("src/test/resources/gtfs/trips.txt")
  ...
  it should "get Some" in {
    load.list.find(_.tripId == TripId("3369:1")) should equal(Some(
  ...
  it should "get None" in {
    load.list.find(_.tripId == TripId("4242:1")) should equal(None)
  }
}
