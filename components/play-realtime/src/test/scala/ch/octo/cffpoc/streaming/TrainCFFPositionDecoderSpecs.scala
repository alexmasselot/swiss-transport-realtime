...
import ch.octo.cffpoc.streaming.serialization.TrainCFFPositionDeserializer
...
class TrainCFFPositionDecoderSpecs extends FlatSpec with Matchers {
  ...
  it should "decode" in {
    ...
    val tp = new TrainCFFPositionDeserializer().deserialize("", bs)
    ...
    current.timedPosition.timestamp.toDateTime(DateTimeZone.UTC) should equal(new DateTime(1455742294099L).toDateTime(DateTimeZone.UTC))

    tp.futurePositions should have length 16
    tp.futurePositions(2).toDateTime(DateTimeZone.UTC) should equal(TimedPosition(new DateTime(1455742294099L).toDateTime(DateTimeZone.UTC).plusMillis(4000), GeoLocBearing(47.547399, 7.589565, Some(50))))
    //None position at 3
    tp.futurePositions(3).toDateTime(DateTimeZone.UTC) should equal(TimedPosition(new DateTime(1455742296099L).toDateTime(DateTimeZone.UTC).plusMillis(4000), GeoLocBearing(47.547399, 7.589565, None)))
  }
}
