...
class TrainCFFPositionDecoderSpecs extends FlatSpec with Matchers {
  ...
  it should "decode" in {
    ...
    current.timedPosition.position should equal(GeoLocBearing(47.547399, 7.589565, 300))
    current.timedPosition.timestamp should equal(new DateTime(1455742294099L))

    tp.futurePositions should have length 16
    tp.futurePositions(2) should equal(TimedPosition(new DateTime(1455742294099L).plusMillis(4000), GeoLocBearing(47.547399, 7.589565, 50)))
  }
}
