...
class TrainPositionSnapshotSpecs extends FlatSpec with Matchers with DateMatchers {
  ...
  def mockTrain(str: String): TrainPosition = {
    ...
    TrainPosition(
      ...
      timedPosition = TimedPosition(timestamp = mockDate(time), position = GeoLocBearing(0, 0, None))
    )
  }
  ...
  it should "mock works" in {
    val t2 = mockTrain("1/x1/12:42:00").at(TimedPosition(mockDate("12:54:00"), GeoLocBearing(12, 102, Some(20))))
    t2.train.id should equal("1")
    t2.timedPosition should equal(TimedPosition(mockDate("12:54:00"), GeoLocBearing(12, 102, Some(20))))
  }
